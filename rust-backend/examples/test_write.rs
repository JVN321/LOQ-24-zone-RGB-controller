...
fn main() {
    ...
    println!("Setting zones 0 to 23 to GREEN...");
    // Report 5 format: [0x05, 0x01, start, 0x00, end, 0x00, R, G, B, 0x01]
    let buf_green = vec![0x05, 0x01, 0x00, 0x00, 0x17, 0x00, 0x00, 0xff, 0x00, 0x01];
    device.send_feature_report(&buf_green).expect("Failed to send green color");
    println!("Green color sent!");
    
    thread::sleep(Duration::from_secs(3));

    println!("Setting zones 0 to 23 to BLUE...");
    let buf_blue = vec![0x05, 0x01, 0x00, 0x00, 0x17, 0x00, 0x00, 0x00, 0xff, 0x01];
